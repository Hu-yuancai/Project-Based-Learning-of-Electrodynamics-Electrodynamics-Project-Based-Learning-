...
这一版是最初始的代码，直接就根据deep seek的一个需求文档给到copilot完成构建，运行大概十分钟，配置了一下环境，得到整个结果，看似很完善，图也很精致，但是问题在于：

**1.黑箱性严重**：物理公式，数据来源，目标设定，使用的算法都不清纯是否正确；

**2.物理参数设定不正确**：现在关于材料性质的设置都是AI直接生成的，不一定正确，关键是目前的深度学习框架误差很大，说明根本没有学习到有效的特征，无法实现逆向设计的功能，这是我们后续要重点改进的点；

**3.目标的设定不清晰**：目前deep seek只是随便给了工作波长和入射角度，但并没有真实的物理场景，只是纯数学上的模型检验，没有落实到实际物理意义上。

**4.物理公式不完善**：推导的广义菲涅尔公式存在简化，直接就给到正入射的情况，没有给到完善的公式，以及公式的文献来源。

上述内容是第一版代码的问题，我们进行详细修改，**多轮、多对象AI交互**，完成对上述问题的·解决。
